...
from gym_app.validators import (
    validate_member_number,
    validate_valid_member_number,
...
class RegisterNewMemberForm(forms.Form):
    """New gym user registration form"""
    member_number = forms.IntegerField(
        max_value=9999999,
        required=True,
        validators=[validate_member_number],
    ...
    date_of_birth = forms.DateField(
        widget=forms.SelectDateWidget(
            years=range(1922, 2022), attrs={"class": "form-control"}
        ),
        required=True,
    )
    ...
    emergency_contact_phone_number = forms.IntegerField(
        min_value=0,
        max_value=999999999,
        required=True,
        widget=forms.NumberInput(attrs={"class": "form-control"}),
...
class OnlineRegistrationForm(forms.Form):
    ...
    member_number = forms.IntegerField(
        max_value=9999999, required=True, validators=[validate_valid_member_number]
    )
...
def get_all_members():
    """Function retrieves list of all members in database"""
    choices = []
...
class ManagePaymentForm(forms.Form):
    """Form to select members for payments"""
    member = forms.ChoiceField(
        label="Select Member", widget=forms.Select(attrs={"class": "form-control"})
    )
...
class ChangePasswordForm(forms.Form):
    """Form to update online user password"""
    current_password = forms.CharField(
        label="Current Password", max_length=20, widget=forms.PasswordInput
    )
    password1 = forms.CharField(
...
class EditProfileForm(forms.Form):
    """Form for member to edit profile information"""
    ...
    emergency_contact_phone_number = forms.IntegerField(
        min_value=0, max_value=999999999, required=True
    )


class EditMemberProfileForm(forms.Form):
    """form for admin to update member information"""
    first_name = forms.CharField(
        max_length=30,
        required=True,
        widget=forms.TextInput(attrs={"class": "form-control"}),
...
class ChangeMembershipNumberForm(forms.Form):
    """Form to change membership number"""
    member = forms.ChoiceField(
        label="Select Member", widget=forms.Select(attrs={"class": "form-control"})
    )
...
class RegisterMemberVisitSelectForm(forms.Form):
    """Form to register member visitation"""
    member = forms.ChoiceField(
        widget=forms.Select(
            attrs={"placeholder": "Select Member", "class": "form-control"}
        ),
        label="Select member",
        required=False,
    )
...
def get_event_hosts():
    """Function to return list of hosts for events"""
    hosts = []
